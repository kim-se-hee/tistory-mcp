...
| `tistory_publish_post`        | `content` (입력 포맷은 `contentFormat` 따름 — `markdown` 입력 시 도구가 HTML 변환), `contentFormat?` (`markdown` 기본 / `html`), `title`, `category?`, `tags?`, `visibility?` (`public`/`private`/`protected`, 기본 `private`), `slogan?`, `password?`, `type?` (`post`/`page`), `published?` (★ 초안 토글 아님 — 0/1 모두 실제 글 생성, docs/api.md §4.5), `attachments?` (영구화 ref 배열 = upload_image 의 `attachmentRef`), `allowNoCategory?` (기본 `true` — `false` 시 category 미지정 발행 거부) | `POST /manage/post.json` fetch 1방. ★ 발행 전 `markdown.ts` 로 본문 변환 — `markdown` 이면 MD→HTML, `html` 이면 sanitize (서버가 마크다운 미렌더라 직접 변환 필수, docs/api.md §4.5). 이미지 치환자 `[##_Image\|...]` 는 변환 중 placeholder 로 보호. 응답 `entryUrl` 에서 postId 추출 반환. **`attachments` 미지정 시 본문 이미지가 orphan → GC → 404** (docs/api.md §5.3.1). ★ **발행 가드 (네트워크 호출 전):** `protected` + `password` 미지정 → 거부 (무비밀번호 시 서버 토큰이 들어가 잠기지 않은 보호글 양산); 실수 전체공개는 `visibility` 디폴트(`private`)가 차단 — `public` 은 명시해야만 나가고 발행 성공 시 응답에 ⚠ 경고 (별도 confirm 플래그 없음); `category` 미지정(0) + `allowNoCategory:false` → 거부 후 `fetch_meta`/`categories_update` 유도 (기본 `true` 라 평소엔 통과 — category 0 은 숨김 아닌 '그룹 미지정', 홈 피드 노출, docs/api.md §4.5). public 발행 성공 시 응답에 ⚠ 경고 명시. ★ 응답에 적용 categoryId(미지정=0 '그룹 미지정' 톤) 항상 표기. ★ **`blogUrl` 필수** (zod `.trim().min(1)` + 빈/공백 거부 가드) — 파괴적 작업이라 `default` 슬롯으로의 조용한 폴백 차단, 응답에 `target host: {ctx.host}` 항상 표기 (오발행 방지) |
| `tistory_update_post`         | `postId` (or `postUrl`), `content` (**필수 — 미지정 시 본문이 빈 문자열로 덮어쓰여 거부**), `contentFormat?` (`html` 기본 / `markdown`), `title?`, `category?`, `tags?`, `visibility?`, `attachments?` (영구화 ref 배열) | `PUT /manage/post/{id}.json` fetch. 숫자 `postId` 직접 제공 + 머지 필드(title/slogan/visibility/category/password) 전부 명시 시 순회 생략하고 PUT path 직행 (docs/api.md §4.6); 일부 생략 시만 현재 메타를 `/manage/posts.json` 페이지 순회 (최대 20p / 300건) 로 매칭 → 인자로 덮어쓰기 → full body PUT. ★ 본문은 `contentFormat` 따라 변환/sanitize 후 박음 (수정은 `html` 기본 — 되박기 가드는 변환 전 raw 에서 검사). **`posts.json` 응답에 `tag` 필드가 없어** 메타 머지로는 태그 복원 불가 — `tags` 미지정 시 도구가 공개 페이지 `a[rel="tag"]` 를 스크레이프해 현재 태그를 자동 보존(빈 문자열로 안 덮음). 보존 실패(비공개/보호글 공개 조회 4xx 등) 시 조용히 비우지 않고 거부(`tags` 직접 지정 유도). 명시적 비우기는 빈 배열 `tags:[]`. 본문 이미지는 `attachments` 로 영구화 ref 를 같이 보내야 보존 (docs/api.md §5.3.1). ★ **`blogUrl` 필수** (빈/공백 거부, `default` 슬롯 폴백 차단), 응답에 `target host` 항상 표기 (오수정 방지) |
| `tistory_delete_post`         | `postId` (or `postUrl`)                                                                                                                                | `DELETE /manage/post/{id}.json` fetch 1방. 숫자 `postId` 직행 (순회 없음); `postUrl`/slogan 만 주어지면 page slogan 우회 위해 `posts.json` 순회로 실제 id 매칭. ★ **`blogUrl` 필수** (빈/공백 거부, `default` 슬롯 폴백 차단), 응답에 `target host` 항상 표기 (오삭제 방지)                                                                                                                                            |
| `tistory_upload_image`        | `filePath`, `filename?`, `mime?`, `width?`, `height?`, `align?`, `verbose?` (기본 `false`)                                                              | `POST /manage/post/attach.json` multipart (field `file`). 응답 `{ key, name, size }` + ready-to-paste `permanentReplacer` (본문 치환자 `[##_Image\|{attachmentRef}\|CDM\|1.3\|{json}_##]`) + **`attachmentRef`** (서명 통째 포함 `kage@{key}/{filename}?{서명query}`, `&`→`&amp;`) 동봉. ★ 발행 시 `publish_post`/`update_post` 의 `attachments` 인자에 이 `attachmentRef` 를 넘겨야 영구화 — 본문 치환자 kage 값과 글자 단위 동일 (docs/api.md §5.3.1). `width`/`height`/`align` 은 치환자 json 메타 (`originWidth`/`originHeight`/`style`). `width`/`height` 미지정 시 **로컬 파일 헤더에서 실픽셀 자동 추출** (PNG/JPEG/GIF/WebP/BMP magic bytes, deps 미추가 — `parseImageDimensions`); 못 구하면 0×0 + `widthOrigin` 폴백 + 경고. `align` 미지정 시 `alignCenter`. 응답 `url`(서명·만료 +15일)은 **기본 비노출** — 본문 직박 시 404 함정이라 `verbose:true` 일 때만 `temporaryUrl` 로 포함 (docs/api.md §5.3.1) |
...
| `tistory_categories_update`   | `tree` (전체 카테고리 트리 — `{ id?, name, visibility?, children[] }[]`, **중첩 children 지원**. id 있으면 기존 노드, 없으면 신규. 누락된 기존 id 는 삭제. 배열 순서 = 표시 순서, nesting = 계층)                                                                                                                                                                                              | 카테고리 CRUD 통합 — `/manage/category` 의 batch 모드와 동일 시맨틱. 현재 트리 (`/manage/category.json` GET) 와 diff → **`PUT /manage/category.json`** 한 방 (cookie-only fetch, 함정 1 유지). body 는 `{ rootLabel, delete[], append[], update[] }` 3-array diff (delete = id 정수 배열, append = `id:-1` 객체, update = `label`에 이전 이름 보존). 응답 = `{ categoryTree: [...] }` → 평탄화 (`{ id, name, parent, depth, priority, entries, visibility, leaf }[]`, 계층은 nesting 위치에서 재구성 — GET/PUT 노드엔 `parent`/`depth`/`opened` 없음). docs/api.md §3.6, **§3.6.1** (실측 확정). **★ 하위/이동/visibility/순서 지원:** 신규 하위는 append `parent` + 부모 update `children` 중첩 미러 + 부모 `leaf:false` 3개 동시 (append.children 단독은 자식 무시 — 실측). visibility 는 update 정수(0/15/20) 토글로 적용. 순서 = 같은 레벨 `priority`(배열 index). 이동(부모 변경) = 새 위치 nesting 으로 환원. 단 신규 부모 밑으로의 **기존 노드 이동은 거부** (부모 id 미정 → 서버가 못 찾음). 한도 500개 / 글 0개 카테고리만 삭제 enable (UI 가드 — fetch 사전 검증). |
...
```
모든 도구
  → 호출 전 세션 체크 (cookie expiry / 401·302 감지 / 200-OK 로그인 페이지 본문 마커 감지)
...
만료 감지 함정: 302→`/auth/login`·401 만으론 부족. 만료 세션이 **200 으로 로그인 페이지 HTML** 을 돌려줄 수 있어(SPA fallback·`www.tistory.com/auth/login` 직렬 렌더) `requestJson` 이 가짜 성공으로 통과시킨다. `api.ts` 는 200 본문에서 로그인 마커(`accounts.kakao.com`, `/auth/login`, `id="loginKey"` 등 — JSON 본문은 제외)를 검사하고, `publishPost`/`updatePost` 는 `entryUrl` 형식(`/{정수}` 또는 `/pages/...`, host 일치)까지 검증해 둘 중 어느 쪽이든 어긋나면 `SessionExpiredError`/`TistoryApiError` 로 변환한다.
...
### 3.4. 본문 MD→HTML 변환 내장 (도구 책임)

★ **티스토리 서버는 본문 마크다운을 렌더하지 않는다** (docs/api.md §4.5 (1) 실측). `# 제목`·`**굵게**` 를 그대로 발행하면 기호가 리터럴로 노출됨 = "마크다운 생노출" 블로커. body 에 "마크다운 모드" 필드도 없음. → **변환은 도구 책임.** `src/tistory/markdown.ts` 가 `marked`(GFM+breaks) 로 MD→HTML 변환 후 `sanitize-html` 로 sanitize 한다.

- **분기:** `publish_post` 는 `contentFormat: markdown` 디폴트 (글 작성은 마크다운이 자연), `update_post` 는 `html` 디폴트 (기존 글 HTML 다듬어 되박는 경우가 흔함). 둘 다 반대 포맷 지정 가능. `html` 경로는 변환 없이 sanitize 만.
- **sanitize 정책:** docs/api.md §4.5 (2) 실측 화이트리스트를 **관대하게** 허용 (h1~h6+id / table / figure·img / pre>code[class] / blockquote / a[href,target,rel] / span[style] / div[class,data-*] / iframe). 헤딩 `id` 는 서버 목차 스크립트가 쓰므로 죽이지 않음. `<script>`/`on*` 핸들러/`javascript:` 스킴 등 위험 벡터만 제거.
- **이미지 치환자 보호:** `[##_Image|{kageRef}|CDM|1.3|{json}_##]` 는 marked/sanitize 가 깨뜨리지 않도록 변환 **전** placeholder 토큰으로 빼두고 **후** 글자 단위 원복. → 치환자 kage 값의 `&amp;`·서명 query 가 손상되지 않아 발행 body `attachments` 와의 글자 일치가 보장됨 (docs/api.md §5.3.1).
